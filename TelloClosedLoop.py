...
###################################################################################################
    ...
    for i in range(0,500):
        ...
        # Compute Reference Signal (this is how we are going to implementing our control)
        reference = 1
        # (2* amplitude/np.pi) * np.arcsin(np.sin((2*np.pi/period)*ptime))
        
        # Compute Error and Control Input
        error = int(reference - yaw) # <-------------------------------------------------------------------------------- Fill this out
        integratedError = integratedError + ptime * error
        errorDerivative = (error-errorStore) / ptime
        errorStore = error
        control_YA = kp *error + ki*integratedError + kd*errorDerivative# <--------------------------------------------------------------------------- Fill this out
